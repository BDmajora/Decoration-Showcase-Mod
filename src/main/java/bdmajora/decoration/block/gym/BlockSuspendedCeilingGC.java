...
import static bdmajora.decoration.block.ModBlocks.suspendedCeilingGC;
import static bdmajora.decoration.block.ModBlocks.suspendedCeilingGCOn;

public class BlockSuspendedCeilingGC extends BlockTransparent {
	public useless.dragonfly.model.block.processed.BlockModel model;
	public BlockSuspendedCeilingGC(String key, int id, Material material, useless.dragonfly.model.block.processed.BlockModel model) {
	...
	public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
		...
		Block newBlock;
		int newMeta;
		if (currentBlock == suspendedCeilingGC) {
			newBlock = suspendedCeilingGCOn;
			newMeta = 1; // 'on' state
		} else { // currentBlock == gymLightOn
			newBlock = suspendedCeilingGC;
			newMeta = 0; // 'off' state
		}
