...
public class BlockCeilingLight extends BlockTransparent {
	public useless.dragonfly.model.block.processed.BlockModel model;
	public BlockCeilingLight(String key, int id, Material material, useless.dragonfly.model.block.processed.BlockModel model) {
	...
	public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
		...
		Block newBlock;
		int newMeta;
		if (currentBlock == ceilingLight) {
			newBlock = ceilingLightOn;
			newMeta = 1; // 'on' state
		} else { // currentBlock == gymLightOn
			newBlock = ceilingLight;
			newMeta = 0; // 'off' state
		}
